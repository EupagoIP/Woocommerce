...
class WC_EuPago_Callback {
  ...
  */
  ...
  public function callback_handler() {
    ...
    // Verificar CHAVE API
    if ( !isset( $_GET['chave_api'] ) || empty( $_GET['chave_api'] ) || $_GET['chave_api'] != $this->integration->get_api() ) {
    ...
    } elseif ( 'shop_order' != get_post_type( $_GET['identificador'] ) ) { // verifica se pertence a uma encomenda
      $this->callback_log('O ID não pertence a uma encomenda', true);
    }

    // object da encomenda
    $order = new WC_Order( $_GET['identificador'] );
    ...
    if ( !isset($_GET['mp'] ) || empty( $_GET['mp']) || !in_array( $payment_method, $this->get_gateway( $_GET['mp'] ) ) ) {
      $this->callback_log('Conflito de Método de Pagamento', true);
    }

    // Confirma se existe referência e coincide com a encomenda
    if ( !isset( $_GET['referencia'] ) || empty( $_GET['referencia'] ) || get_post_meta( $order->get_id(), '_'.$payment_method.'_referencia', true ) != $_GET['referencia'] ) {
    ...
    if ( !isset( $_GET['valor'] ) || empty( $_GET['valor'] ) ) {
      ...
      if ( !($order_total == $_GET['valor']) && !apply_filters( 'eupago_for_woocommerce_callback_value_check', false, $order, $_GET['valor'] ) ) {
        $this->callback_log('Erro no Valor', true);
      }
    }

    $note = 'Pagamento Efectuado ';
    if ( isset( $_GET['data'] ) ) $note.= '<br />Data/Hora: ' . $_GET['data'];
    if ( isset( $_GET['local'] ) ) $note.= '<br />Local: ' . $_GET['local'];
    if ( isset( $_GET['transacao'] ) ) $note.= '<br />Transação: ' . $_GET['transacao'];
    $order->add_order_note( $note );
    $order->payment_complete( $_GET['transacao'] );
