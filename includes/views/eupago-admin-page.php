...
function eupago_page_content() {
	if (isset($_POST['eupago_save'])) {
		$channel 					= $_POST['channel'];
		$api_key 					= $_POST['api_key'];
		$endpoint 					= $_POST['endpoint'];
		$reminder 					= isset($_POST['reminder']) ? $_POST['reminder'] : '0';
		$debug 						= isset($_POST['debug']) ? $_POST['debug'] : 'no';
		$client_id 					= $_POST['client_id'];
		$client_secret 				= $_POST['client_secret'];
		$user_eupago 				= $_POST['user_eupago'];
		$password_eupago 			= $_POST['password_eupago'];
		$sms_enable 				= isset($_POST['sms_enable']) ? $_POST['sms_enable'] : 'no';
		$sms_payment_hold 			= isset($_POST['sms_payment_hold']) ? $_POST['sms_payment_hold'] : 'no';
		$sms_payment_confirmation 	= isset($_POST['sms_payment_confirmation']) ? $_POST['sms_payment_confirmation'] : 'no';
		$sms_order_confirmation 	= isset($_POST['sms_order_confirmation']) ? $_POST['sms_order_confirmation'] : 'no';
		$sms_intelidus_id 			= $_POST['sms_intelidus_id'];
		$sms_intelidus_api 			= $_POST['sms_intelidus_api'];
		$intelidus_sender 			= $_POST['intelidus_sender'];
	...
	<div class="eupago_header">
		<div>
			<img src="<?php echo site_url() . '/wp-content/plugins/eupago-gateway-for-woocommerce/assets/images/dekas_home.png'; ?>" alt="Dekas">
		</div>
		<div>
	...
	<div class="eupago-settings">
		<form name="eupago-settings" method="POST" action="">
			<table class="form-table" role="presentation">
				<tbody>
					<tr>
						...
						<td>
							<input class="regular-text" type="text" name="channel" value="<?php echo get_option( 'eupago_channel' ); ?>">
							<h4><?php echo __('What is a channel?', 'eupago-gateway-for-woocommerce'); ?></h4>
							<p><?php echo __("Each account has at least a channel. Each channel has an API Key that identifies your euPago's account.", 'eupago-gateway-for-woocommerce'); ?></p>
							<p>
								<?php echo __("In order to find your API Key and channel name please follow this guide on our", 'eupago-gateway-for-woocommerce'); ?> 
								<a href="https://eupago.atlassian.net/servicedesk/customer/portal/2/article/224297034?src=1875300770" target="_BLANK"><?php echo __("Help Center", 'eupago-gateway-for-woocommerce'); ?></a>.
							</p>
						</td>
					</tr>
					<tr>
						...
						<td>
							<input class="regular-text" type="text" name="api_key" value="<?php echo get_option( 'eupago_api_key' ); ?>">
							<p>
								<?php 
						...
						<td>
							<select name="endpoint">
								<?php if (get_option('eupago_endpoint') == 'clientes') { ?>
									<option value="clientes" selected><?php echo __( 'Live', 'eupago-gateway-for-woocommerce' ); ?></option>
								<?php } else { ?>
									<option value="clientes"><?php echo __( 'Live', 'eupago-gateway-for-woocommerce' ); ?></option>
								<?php } ?>
								<?php if (get_option('eupago_endpoint') == 'sandbox') { ?>
									<option value="sandbox" selected><?php echo __( 'Sandbox', 'eupago-gateway-for-woocommerce' ); ?></option>
								<?php } else { ?>
									<option value="sandbox"><?php echo __( 'Sandbox', 'eupago-gateway-for-woocommerce' ); ?></option>
								<?php } ?>
							</select>
						</td>
					</tr>
					<tr>
						...
						<td>
							<input type="checkbox" name="reminder" value="1" <?php echo $reminder_checked; ?>><?php echo __( 'Enable', 'woocommerce' ); ?>
							<p>
								<?php echo __( 'Do you want to send a reminder to your customer to inform him he has a pending order? Activate this option. Read more about this reminder', 'eupago-gateway-for-woocommerce' ); ?> 
								<a href="https://eupago.atlassian.net/servicedesk/customer/portal/2/article/652967937" target="_BLANK"><?php echo __( 'here', 'eupago-gateway-for-woocommerce' ); ?></a>
							</p>
						</td>
					</tr>
					<tr>
						...
						<td>
							<input type="checkbox" name="debug" value="yes" <?php echo $debug_checked; ?>><?php echo __( 'Enable logging', 'woocommerce' ); ?>
							<p>
								<?php echo __( 'Log plugin events, such as callback requests, inside', 'eupago-gateway-for-woocommerce' ); ?>
								<code><?php echo site_url() . '/wp-content/uploads/wc-logs/'; ?></code>
							</p>
						</td>
					</tr>
				</tbody>
			...
			<h3><?php echo __('Refund', 'eupago-gateway-for-woocommerce'); ?></h3>
			<table class="form-table" role="presentation">
				<tbody>
					<tr>
						<th scope="row"><label for="client_id"><?php esc_html_e( 'Client ID:', 'eupago-gateway-for-woocommerce' ); ?></label></th>
						<td><input class="regular-text" type="text" name="client_id" value="<?php echo get_option( 'eupago_client_id' ); ?>"></td>
					</tr>
					<tr>
						<th scope="row"><label for="client_secret"><?php esc_html_e( 'Client Secret:', 'eupago-gateway-for-woocommerce' ); ?></label></th>
						<td><input class="regular-text" type="text" name="client_secret" value="<?php echo get_option( 'eupago_client_secret' ); ?>"></td>
					</tr>
					<tr>
						<th scope="row"><label for="user_eupago"><?php esc_html_e( 'User:', 'eupago-gateway-for-woocommerce' ); ?></label></th>
						<td><input class="regular-text" type="text" name="user_eupago" value="<?php echo get_option( 'eupago_user' ); ?>"></td>
					</tr>
					<tr>
						<th scope="row"><label for="password_eupago"><?php esc_html_e( 'Password:', 'eupago-gateway-for-woocommerce' ); ?></label></th>
						<td><input class="regular-text" type="password" name="password_eupago" value="<?php echo get_option( 'eupago_password' ); ?>"></td>
					</tr>
				</tbody>
			</table>

			<h3><?php echo __('SMS Intelidus360', 'eupago-gateway-for-woocommerce'); ?></h3>
			<table class="form-table" role="presentation">
				<tbody>
					<tr >
						<th scope="row"><label for="sms_enable"><?php esc_html_e( 'SMS Notifications:', 'eupago-gateway-for-woocommerce' ); ?></label></th>
						<td><input type="checkbox" name="sms_enable" value="yes" <?php echo $sms_enable_checked; ?>><?php echo __( 'Enable', 'woocommerce' ); ?></td>
					</tr>
					<?php
					...
					?>
					<tr class="<?php echo $sms_enabled; ?>">
						<th scope="row"><label for="sms_payment_hold"><?php esc_html_e( 'SMS Payment On Hold:', 'eupago-gateway-for-woocommerce' ); ?></label></th>
						<td><input type="checkbox" name="sms_payment_hold" value="yes" <?php echo $sms_payment_hold_checked; ?>><?php echo __( 'Enable', 'woocommerce' ); ?></td>
					</tr>
					<tr class="<?php echo $sms_enabled; ?>">
						...
						<td><input type="checkbox" name="sms_payment_confirmation" value="yes" <?php echo $sms_payment_confirmation_checked; ?>><?php echo __( 'Enable', 'woocommerce' ); ?></td>
					</tr>
					<tr class="<?php echo $sms_enabled; ?>">
						...
						<td><input type="checkbox" name="sms_order_confirmation" value="yes" <?php echo $sms_order_confirmation_checked; ?>><?php echo __( 'Enable', 'woocommerce' ); ?></td>
					</tr>
					<tr class="<?php echo $sms_enabled; ?>">
						<th scope="row"><label for="sms_intelidus_id"><?php esc_html_e( 'SMS Intelidus 360 ID:', 'eupago-gateway-for-woocommerce' ); ?></label></th>
						<td><input class="regular-text" type="text" name="sms_intelidus_id" value="<?php echo get_option( 'eupago_sms_intelidus_id' ); ?>"></td>
					</tr>
					<tr class="<?php echo $sms_enabled; ?>">
						<th scope="row"><label for="sms_intelidus_api"><?php esc_html_e( 'SMS Intelidus 360 API:', 'eupago-gateway-for-woocommerce' ); ?></label></th>
						<td><input class="regular-text" type="text" name="sms_intelidus_api" value="<?php echo get_option( 'eupago_sms_intelidus_api' ); ?>"></td>
					</tr>
					<tr class="<?php echo $sms_enabled; ?>">
						<th scope="row"><label for="intelidus_sender"><?php esc_html_e( 'Intelidus 360 Sender:', 'eupago-gateway-for-woocommerce' ); ?></label></th>
						<td><input class="regular-text" type="text" name="intelidus_sender" value="<?php echo get_option( 'eupago_intelidus_sender' ); ?>"></td>
					</tr>
				</tbody>
			</table>
			<p>
		...
		<div class="eupago-sidebar">
			<img src="<?php echo site_url() . '/wp-content/plugins/eupago-gateway-for-woocommerce/assets/images/eupago_logo.png'; ?> " alt="Eupago Logo">
			<p><?php echo __("In order to use euPago's plugin for WooCommerce you must have an euPago's account.", 'eupago-gateway-for-woocommerce'); ?></p>
			<p><?php echo __('Do you need an account? You may sign up at', 'eupago-gateway-for-woocommerce'); ?> <a href="https://www.eupago.pt/registo" target="_BLANK">https://www.eupago.pt/registo</a>.</p>
			<p><?php echo __('Do you already have a demo account and need to finish your real account? Please reach out by email:', 'eupago-gateway-for-woocommerce'); ?>  <a href="mailto:comercial@eupago.pt">comercial@eupago.pt</a>.</p>
		</div>
	</div>
<?php
}
