    ...
    <?php if ($this->id == 'eupago_cc') { ?>
      <?php if (!extension_loaded('soap')) { ?>
        <div class="eupago-soap-error">
          <p><?php echo __('Requires SOAP protocol active on your server.', 'eupago-gateway-for-woocommerce'); ?></p>
        </div>
      <?php } ?>

      <h4><?php echo __('How can I provide this payment method?', 'eupago-gateway-for-woocommerce'); ?></h4>
      <p>
        <?php echo __('You must first request its activation on your account by email:', 'eupago-gateway-for-woocommerce'); ?> 
        <a href="mailto:comercial@eupago.pt" target="_BLANK">comercial@eupago.pt</a>
      </p>
      <p><?php echo __('This request is subject to the approval of euPago\'s compliance department.', 'eupago-gateway-for-woocommerce'); ?></p>
    <?php } ?>
